...
VALUES
  (
    ...
    FOREIGN KEY (fk_cou_id) REFERENCES course(cou_id) ON
             DELETE CASCADE
  );
  ...
  CREATE TABLE "finished_class" (
    ...
    FOREIGN KEY (fk_cla_id) REFERENCES "class"(cla_id) ON
                   DELETE CASCADE,
    fk_use_id INT NOT NULL,
    FOREIGN KEY (fk_use_id) REFERENCES "user"(use_id) ON
                   DELETE CASCADE
  );
INSERT INTO
  class (
...
VALUES
  (
    'Aula 1',
    '50',
    'Descrição qualquer de uma aula',
    'wY4xqCjVZ-M',
    1
  );
INSERT INTO
...
CREATE
  OR replace FUNCTION get_user_id_by_email(fun_use_email_or_nick VARCHAR) returns bigint AS $$ BEGIN RETURN (
    SELECT
      use_id
    FROM
      "user"
    WHERE
      use_email = fun_use_email_or_nick
      OR use_nickname = fun_use_email_or_nick
    limit
      1
  );
...
select
	get_user_id_by_email('caique@email.com');

select
	check_password('caique@email.com',
	'123456');
